...
import { fetch, update, deleteItem, setRefetch, deleteItemsByIds } from '../../stores/employees/employeesSlice'
...
const TableSampleEmployees = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  const { employees, loading, count, notify: employeesNotify, refetch } = useAppSelector((state) => state.employees)
  ...
  useEffect(() => {
      if (employeesNotify.showNotification) {
          notify(employeesNotify.typeNotification, employeesNotify.textNotification);
      }
  ...
  useEffect(() => {
    loadColumns(handleDeleteModalAction, `employees`).then((newCols) =>
      setColumns(newCols),
    );
  }, []);
